...
----------------------------------------------------------------------------------------
## Generate SBOM files for WRLinux LTS25 and later releases

Starting with WRLinux LTS25, SPDX 3.0 SBOM files are generated by default, so this layer is no longer required.

To ensure the generated SBOM files are compatible with [Wind River Scanning Tool](https://studio.windriver.com/scan), you must enable the `feature/purl` template. You can do this in one of the following ways:

- Add `--templates feature/purl` when setting up the project, or
- Append the template in `local.conf`:

```bash
echo 'WRTEMPLATE:append = " feature/purl"' >> ./conf/local.conf
```
