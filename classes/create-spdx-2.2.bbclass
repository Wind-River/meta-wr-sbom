...
def add_package_files(d, doc, spdx_pkg, topdir, get_spdxid, get_types, *, archive=None, ignore_dirs=[], ignore_top_level_dirs=[]):
    ...
    if check_compiled_sources:
        compiled_sources, types = oe_sbom.spdx_common.get_compiled_sources(d)
        bb.debug(1, "Total compiled files: %d" % len(compiled_sources))
    for subdir, dirs, files in os.walk(str(topdir)):
        ...
        for file in files:
            ...
            if not filepath.is_symlink() and filepath.is_file():
                ...
                if check_compiled_sources:
                     if not oe_sbom.spdx_common.is_compiled_source(filename, compiled_sources, types):
                          continue
                ...
                if "SOURCE" in spdx_file.fileTypes:
                    extracted_lics = oe_sbom.spdx_common.extract_licenses(filepath)
                    if extracted_lics:
...
def collect_dep_recipes(d, doc, spdx_recipe):
    ...
    deps = oe_sbom.spdx_common.get_spdx_deps(d)
    ...
    for dep in deps:
        ...
        dep_recipe_path = oe_sbom.sbom.doc_find_by_hashfn(deploy_dir_spdx, package_archs, "recipe-" + dep.pn, dep.hashfn)
...
def add_download_packages(d, doc, recipe):
    ...
    for download_idx, src_uri in enumerate(d.getVar('SRC_URI').split()):
        ...
        package = oe_sbom.spdx.SPDXPackage()
        package.name = "%s-source-%d" % (d.getVar("PN"), download_idx + 1)
        package.SPDXID = oe_sbom.sbom.get_download_spdxid(d, download_idx + 1)
        ...
        if f.method.supports_checksum(f):
            for checksum_id in CHECKSUM_LIST:
                if checksum_id.upper() not in oe_sbom.spdx.SPDXPackage.ALLOWED_CHECKSUMS:
                    continue
                ...
                c = oe_sbom.spdx.SPDXChecksum()
                c.algorithm = checksum_id.upper()
                c.checksumValue = expected_checksum
                package.checksums.append(c)

        package.downloadLocation = oe_sbom.spdx_common.fetch_data_to_uri(f, f.name)
...
python do_create_spdx() {
    ...
    cpe_ids = oe_sbom.cve_check.get_cpe_ids(d.getVar("CVE_PRODUCT"), d.getVar("CVE_VERSION"))
    if cpe_ids:
    ...
    doc_sha1 = oe_sbom.sbom.write_doc(d, doc, pkg_arch, "recipes", indent=get_json_indent(d))
    ...
    if not recipe_spdx_is_native(d, recipe):
        ...
        for package in d.getVar("PACKAGES").split():
            ...
            oe_sbom.sbom.write_doc(d, package_doc, pkg_arch, "packages", indent=get_json_indent(d))
}
...
python do_create_runtime_spdx() {
    ...
    providers = oe_sbom.spdx_common.collect_package_providers(d)
    pkg_arch = d.getVar("SSTATE_PKGARCH")
    package_archs = d.getVar("SPDX_MULTILIB_SSTATE_ARCHS").split()
    package_archs.reverse()
    ...
    if not is_native:
        ...
        for package in d.getVar("PACKAGES").split():
            ...
            pkg_spdx_path = oe_sbom.sbom.doc_path(deploy_dir_spdx, pkg_name, pkg_arch, "packages")
            ...
            oe_sbom.sbom.write_doc(d, runtime_doc, pkg_arch, "runtime", spdx_deploy, indent=get_json_indent(d))
}
...
python image_combine_spdx() {
    ...
    img_spdxid = oe_sbom.sbom.get_image_spdxid(image_name)
...
def sdk_combine_spdx(d, sdk_type):
    ...
    sdk_spdxid = oe_sbom.sbom.get_sdk_spdxid(sdk_name)
...
def combine_spdx(d, rootfs_name, rootfs_deploydir, rootfs_spdxid, packages, spdx_workdir):
    ...
    if packages:
        for name in sorted(packages.keys()):
            ...
            runtime_spdx_path = oe_sbom.sbom.doc_find_by_hashfn(deploy_dir_spdx, package_archs, "runtime-" + name, pkg_hashfn)
        ...
        with tarfile.open(fileobj=f, mode="w|") as tar:
            def collect_spdx_document(path):
                ...
                for ref in doc.externalDocumentRefs:
                    ref_path = oe_sbom.sbom.doc_find_by_namespace(deploy_dir_spdx, package_archs, ref.spdxDocument)
                    if not ref_path:
