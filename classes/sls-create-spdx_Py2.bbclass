        ...
        for debugsrc in file_data["debugsrc"]:
            ...
            for search in debug_search_paths:
                ...
                if file_sha256 in sources:
                    ...
                    bb.debug(1, "Debug source %s with SHA256 %s not found in any dependency" % (unicode(debugsrc_path), file_sha256))
                break
            else:
...
python do_create_spdx() {
    ...
    found_licenses = dict((license.name, recipe_ref.externalDocumentId + ":" + license.licenseId) for license in doc.hasExtractedLicensingInfos)
...
do_rootfs[postfuncs] = 'image_combine_spdx'
python image_combine_spdx() {
    ...
    for name in sorted(packages.keys()):
        ...
        if not os.path.exists(runtime_spdx_path):
            continue
    ...
    os.symlink(image_spdx_path, image_spdx_link)
