...
def extract_licenses(filename):
    ...
    try:
        ...
        bb.warn("Exception reading {}: {}".format(filename, e))
    return []
...
class Dep(object):
    ...
    def __init__(self, pn, hashfn, in_taskhash):
        self.pn = pn
        self.hashfn = hashfn
        self.in_taskhash = in_taskhash
...
def collect_direct_deps(d, dep_task):
    ...
    for this_dep in taskdepdata.values():
        ...
        bb.fatal("Unable to find this {}:{} in taskdepdata".format(pn, current_task))
    ...
    for dep_name in this_dep[3]:
        ...
        if dep_data[1] == dep_task and dep_data[0] != pn:
            deps.add((dep_data[0], dep_data[6], dep_name in this_dep[8]))
...
def get_patched_src(d):
    ...
    try:
        ...
        if is_work_shared_spdx(d):
            ...
            bb.note("copyhardlinktree {} to {}".format(share_src, src_dir))
...
def get_compiled_sources(d):
    ...
    for src in sources:
        ...
        if ext not in types and ext:
            types.add(ext)
    bb.debug(1, "Num of sources: {} and types: {} {}".format(len(sources), len(types), str(types)))
